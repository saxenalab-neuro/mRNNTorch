"""Interactive flow-field visualizer for leaky mRNN dynamics."""
...
class mFlowFieldVisualizer(AxisSelectionMixin, FlowFieldVisualizerBase):
    ...
    def __init__(
        ...
        """Initialize a leaky mRNN flow-field visualizer.

        Args:
            rnn: Leaky mRNN whose state dynamics will be visualized.
            ...
            fit_states: Full recurrent states used to fit the current 2D PCA plane.
    ...
    def build_finder(self) -> mFlowFieldFinder:
        """Build a leaky flow-field finder for the current region selection.
    ...
    def prepare_data(
        ...
        Args:
            ...
            states: Leaky pre-activation or hidden-state sequence aligned with
                ``inputs``.
            delta_inputs: Optional input perturbations for linear flow fields.
            delta_state_static: Optional perturbations for excluded recurrent
                regions.
        ...
        delta_state_static_nxd = (
            FlowFieldFinderBase._nxd(delta_state_static)
    ...
    def compute_flow_field(
        ...
        delta_state_static_nxd: torch.Tensor | None = None,
        x_is_h: bool = False,
    ) -> FlowField:
        ...
        Args:
            ...
            states_nxd: Flattened leaky states.
            ...
            delta_state_static_nxd: Optional flattened static-region perturbations.
            x_is_h: Whether nonlinear flow should treat the supplied state as
                hidden activity instead of pre-activation state.
        ...
        delta_state_static_n = (
            delta_state_static_nxd[self.current_element_idx]
            if delta_state_static_nxd is not None
            else None
        )
        ...
        with torch.no_grad():
            if self.flow_type == "linear":
                ...
                flow = finder.find_linear_flow(
                    state_n, inp_n, delta_inp_n, delta_state_static=delta_state_static_n
                )[0]
            else:
                flow = finder.find_nonlinear_flow(
                    state_n, inp_n, stim_input=stim_input, W=W, x_is_h=x_is_h
                )[0]
    ...
    def visualize(
        ...
        """Run the interactive leaky flow-field visualization loop.

        Args:
            inputs: Input sequence or batch of sequences shown page by page.
            states: Pre-activation states by default, or hidden activities when
                ``x_is_h`` is true.
            stim_input: Optional additive stimulus used by nonlinear flow fields.
            W: Optional recurrent weight matrix override.
            delta_inputs: Optional input perturbations for linear flow fields.
            delta_state_static: Optional perturbations for excluded recurrent
                regions.
            x_is_h: Forwarded to nonlinear flow computation to interpret the
                supplied state coordinates.
        ...
        """
        inp_nxd, states_nxd, delta_inp_nxd, delta_state_static_nxd = self.prepare_data(
            inputs,
            states,
        ...
        while self.running:
            ...
            if self._flow_dirty or self._flow_cache is None:
                self.compute_flow_field(
                    ...
                    delta_state_static_nxd=delta_state_static_nxd,
                    x_is_h=x_is_h,
                )
