...
def collect_trajectories(model, trial_count=None, time_steps=None):
    ...
    x0 = torch.zeros(trial_count, n_hidden)
    with torch.no_grad():
...
def plot_region_pca(model, h_traj):
    ...
    for name, regions in groups:
        ...
        ax.set(
            title=("" if len(regions) == 1 else
                   f"{name.replace('_', ' ').capitalize()} activity trajectories (PCA)"),
            xlabel="PC1", ylabel="PC2", zlabel="PC3",
        )
        save_figure(fig, "region_pca", f"{name}_activity_trajectories_pca.png")


def find_fixed_points(model, state_traj, optimize_h=False, region=None, static_state_t=None,
                      external_input=None, search_n_inits=None, search_max_iters=None, lr_init=1e-4, initial_state_traj=None, init_noise=0.1):
    ...
    init_noise sets the initial-state noise scale (default: 0.1).
    lr_init sets the optimizer learning rate (default: 1e-4).
    """
    initial_state_traj = state_traj if initial_state_traj is None else initial_state_traj
    ...
    if region is None:
        ...
        excluded = (
            model.rnn.get_excluded_hid_regions(region)
            if static_state_t is None else []
        )
        init_states = fp_finder.region_initial_states(
            initial_state_traj, search_n_inits, region, static_state_t=static_state_t,
            static_states=state_traj,
            noise_scale=init_noise, excluded_static_regions=excluded,
        )
    ...
    unique_fps, all_fps = fp_finder.find_fixed_points(
        init_states, ext_inp, *regions, optimize_h=optimize_h
    )
...
def plot_fixed_points(
    ...
    for trial in traj_proj:
        if len(traj_proj) == 1:
            ax.plot(
                *trial.T, color="black", linewidth=3, solid_capstyle="round",
                path_effects=[
                    path_effects.SimpleLineShadow(
                        offset=(2, -2), shadow_color="black", alpha=0.3,
                        linewidth=6,
                    ),
            ...
            for point, marker, label in (
                (trial[0], "^", "start"), (trial[-1], "X", "end")
            ):
                ax.plot(
                    [point[0]], [point[1]], [point[2]], linestyle="none",
                    marker=marker, markersize=13, markerfacecolor="black",
...
def analyze_activity_fixed_points(model, x_traj, h_traj, init_noise=0.1):
    """Use an activity residual while still optimizing pre-activation states."""
    unique_fps, all_fps = find_fixed_points(model, x_traj, optimize_h=True, init_noise=init_noise)
...
def analyze_nonlinear_flow(model, state_traj, fp_states):
    ...
    ax.scatter(
        *fp_proj_2d.T, marker="*", s=180,
        facecolors="black", edgecolors="white", linewidths=1.2, zorder=5,
    )
    ax.set(
...
def analyze_stability(model, state_traj, fp_states, region=None, label=None, external_input=None, trajectory_end_t=None):
    ...
    plot_fixed_points(
        ...
        title=("" if region else
               "Fixed-point stability: blue stable, red unstable or marginal"),
        stability=stability,
        analysis="stability",
        trajectory_end_t=trajectory_end_t,
        filename=(f"{label}_stability_pca.png" if label else
                  f"{region}_isolated_stability_pca.png" if region else None),
    )
    return stability, eigenvectors


def analyze_linear_flow(
    model, state_traj, fp_states, stability, region=None, label=None,
    excluded_static_regions=None, external_input=None,
):
    ...
    ) if excluded_static_regions is None else excluded_static_regions
    filename = (
        f"{region}_isolated_linear_flow_near_fixed_points_pca.png"
        if region else "linear_flow_near_fixed_points_pca.png"
    )
    if label:
    ...
    fp_proj_2d = flow_finder.transform(model.rnn.get_region_activity(fp_states, *regions))
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.set(title="" if region else f"Fixed point {i + 1}", xlabel="PC1", ylabel="PC2")
    if region is None:
...
def analyze_isolated_region(model, state_traj, region, init_noise=0.1):
    ...
    """
    unique_fps, all_fps = find_fixed_points(model, state_traj, region=region, init_noise=init_noise)
...
def analyze_region_with_static_input(
    model, state_traj, trial_index=0, *, trial_inputs=None,
    initial_state_traj=None,
    include_external_input=False, time_steps=7, search_n_inits=16,
    search_max_iters=2000,
    region="exc",
    lr_init=1e-4,
    init_noise=0.1,
):
    ...
    if include_external_input and trial_inputs.shape[:2] != state_traj.shape[:2]:
        raise ValueError("trial_inputs must match the state trajectories' trials and times")
    condition = "with_external_input" if include_external_input else "zero_external_input"
    results = []
    for t in range(len(trial_states)):
        ...
        external_input = (
            ...
            if include_external_input else trial_states.new_zeros(n_bits)
        )
        print(f"Analyzing {label}")
        unique_fps, all_fps = find_fixed_points(
            model, trial_states, region=region, static_state_t=t,
            external_input=external_input, search_n_inits=search_n_inits,
            search_max_iters=search_max_iters,
            lr_init=lr_init, init_noise=init_noise,
        ...
        stability, _ = analyze_stability(
            model, plot_states, fp_states, region=region, label=label,
            external_input=external_input,
            trajectory_end_t=t,
        )
        analyze_linear_flow(
            model, plot_states, fp_states, stability, region=region, label=label,
            excluded_static_regions=[], external_input=external_input,
        )
...
def analyze_excitatory_with_static_inhibition(
    ...
    return analyze_region_with_static_input(
        model, state_traj, trial_index, region="exc",
        initial_state_traj=initial_state_traj,
        trial_inputs=trial_inputs, include_external_input=include_external_input,
        time_steps=time_steps, search_n_inits=search_n_inits,
        search_max_iters=search_max_iters, lr_init=lr_init, init_noise=init_noise,
    )
...
def analyze_inhibitory_with_static_excitation(
    ...
    return analyze_region_with_static_input(
        model, state_traj, trial_index, region="inhib",
        initial_state_traj=initial_state_traj,
        trial_inputs=trial_inputs, include_external_input=include_external_input,
        time_steps=time_steps, search_n_inits=search_n_inits,
        search_max_iters=search_max_iters, lr_init=lr_init, init_noise=init_noise,
...
def main():
    ...
    for include_external_input in (False, True):
        analyze_excitatory_with_static_inhibition(
            model, short_states, trial_inputs=short_inputs,
